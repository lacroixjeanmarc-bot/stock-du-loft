...
if ($about.Contains("handleCopyEmail")) {
    # Already has the function, now check if onClick is on the email box
    if ($about.Contains("upgrade-modal-mailto-link")) {
        Write-Host "  -> Deja patche (mailto), skip." -ForegroundColor Gray
    } else {
        # Add onClick to email box
        $about = $about.Replace(
            '<div className="upgrade-modal-email-box">',
            '<div className="upgrade-modal-email-box" onClick={handleCopyEmail} style={{ cursor: ''pointer'' }}>'
        )
        
        # Add copy indicator + mailto link after email address
        $oldEmailClose = '<span className="upgrade-modal-email-address">lacroix.jeanmarc@gmail.com</span>
              </div>'
        $newEmailClose = @'
...
'@
        $about = $about.Replace($oldEmailClose, $newEmailClose)
...
# ---- PATCH 3: styles.css - Admin light theme ----
...
if ($css.Contains("ADMIN PAGE — LIGHT OVERRIDE")) {
...
if ($css.Contains("UPGRADE MODAL — EMAIL COPY")) {
...
Remove-Item "patch-and-deploy.ps1" -ErrorAction SilentlyContinue
